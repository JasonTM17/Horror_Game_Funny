...
This document separates implemented design from validation. Runtime telemetry measures fresh Lobby-to-credits timing, but no recorded physical blind run proves the intended 15–20 minute target. The owner accepted that unverified risk for project closure on 2026-07-19.
...
These are design targets. Duration, clarity, comfort, and subjective tension were not manually observed and are not verified claims.
...
These values remain authored goals, not release proof. No dated physical blind keyboard-and-mouse boot-to-credits capture paired with a same-run payload exists; that optional future QA would be required before claiming the 15–20 minute target as verified.
...
The three-memory suite stays authored and story-ordered. Each scare follows anticipation → reveal → aftermath: spatial warning tones and light response first, then a non-colliding silhouette (when used) that faces the player, camera jolt when comfort shake is enabled, and a short monitor-safe fear-vignette pulse. The photograph moves a low whisper from left to right around its message and briefly colors the nearest light. The cassette waits for the camera to turn away, places a non-colliding silhouette behind the player, then layers a breath and two reveal cues when the player looks back; an unrevealed silhouette is removed when `memory_cassette_recalled` completes. The rabbit uses a quiet music-box warning, a red light response, and a non-colliding apparition/presence reveal that faces the player. These beats do not select randomly or change progression beyond their existing memory actions.
...
The family recording must finish before the drawing can be inspected. The player then searches the child's bed, wardrobe, and family table for three physical clues; all five Room 407 clue interactions are required before the final note opens. Wallpaper panels, ceiling ribs, height marks, and a warning label sell the impossible childhood room while keeping the central navigation lane clear. Closing the note starts the climax with a low wall-breath warning and local light response, then reveals a visible, eyed, non-colliding manifestation that faces the player, flashes emission eyes, layers low/sting cues, and spikes camera shake plus fear vignette. The sequence clears before chase ownership starts; the `chase_start` checkpoint is created after the narrative beat completes.

Across the four story-aligned buildup beats—floor arrival, photograph, cassette turn-away, and rabbit—and the Room 407 climax, the direction pattern is anticipation → reveal → aftermath. Volumes remain low/moderate in source and spatial range stays bounded through the audio service, but audible balance and rendered timing/quality were not physically reviewed.
...
No human playthrough or pacing/chase/audio/visual/input/Settings/fullscreen perceptual pass occurred. The owner waived that phase for project closure. Optional future QA can still cover:
