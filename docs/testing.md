...
Four reviewed staged artifact PNGs were prepared as 960×540 RGBA documentation display derivatives with ImageMagick 7.1.2-23 Q16-HDRI x64 (20260517): [lobby](./screenshots/room-407-lobby.png), [Room 407 approach](./screenshots/room-407-bedroom.png), [chase entity](./screenshots/room-407-chase-entity.png), and [ending reveal](./screenshots/room-407-ending-reveal.png). Each uses `-channel RGB -evaluate Pow 0.55 +channel -strip`, a deterministic midtone/shadow lift that adds or removes no scene content; the four files total 2,618,043 bytes. The [640×360 visual-reference GIF](./screenshots/room-407-gameplay-tour.gif) preserves the FFmpeg 8.1.1 source-montage provenance (`palettegen=max_colors=96`, `paletteuse=dither=sierra2_4a`) but was not rederived by FFmpeg for this refresh. Its 59 frames retain the 12/13 cs delays for 7.38 seconds, and ImageMagick repackaged it with `-coalesce -loop 1 -layers Optimize` without a `NETSCAPE2.0` loop extension, so it plays once. The four PNGs and GIF total 5,469,084 bytes (5.22 MiB); together with the cover, committed documentation media total 6,468,515 bytes (6.17 MiB). GDScript did not generate the GIF. Source AVI, all eight current source PNGs, palettes, and logs remain ignored artifacts.
