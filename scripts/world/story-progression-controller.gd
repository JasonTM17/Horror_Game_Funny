...
func get_prompt(action_id: String, _actor: Node) -> String:
	...
	if action_id == "hallway_loop" and memory_count < 3:
		return "[E] Enter the changed hallway" if memory_count > loop_iteration and _latest_memory_recalled() else "Listen to what the memory reveals"
...
func _enter_hallway_loop(actor: Node) -> bool:
	if memory_count <= loop_iteration or memory_count >= 3 or loop_transitioning or not _latest_memory_recalled():
		return false
	loop_iteration += 1
...
func _on_narrative_finished(flag: String) -> void:
	match flag:
		...
		"room_record_heard": GameState.set_objective("A child's drawing is pinned deeper inside Room 407.")
		"chase_ready":
