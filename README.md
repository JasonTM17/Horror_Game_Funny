...
Run all seven headless checks from the repository root:
...
The runner writes one log per check to `.artifacts/test-<name>.log` and isolates Godot user data under `.tmp/`, so it does not overwrite the normal `user://room407.cfg`. It covers import, boot and gameplay loading, state/checkpoint behavior, director-level progression semantics, layout/navigation/chase invariants, and settings/audio structure. It does not prove a 15–20 minute run, visual or audio balance, audible device output, settings persistence across relaunch, or full physical keyboard-and-mouse traversal. See [Testing](docs/testing.md) for the exact matrix.
...
Keep changes focused, use Conventional Commit messages, and do not commit `.godot/`, `.artifacts/`, local tools, exports, or credentials. Before submitting a change, run the seven-check suite and `git diff --check`, then update documentation only for behavior the current source or recorded manual evidence proves.
