...
function Invoke-GodotCheck([string[]]$Arguments, [string]$Name, [string]$Expected = "") {
    ...
    if (Select-String -Path $log -Pattern "ERROR:|SCRIPT ERROR|Parse Error|PROGRESSION_ASSERT|LAYOUT_ASSERT|PHYSICAL_ROUTE_ASSERT|PLAYER_INPUT_ASSERT|SETTINGS_AUDIO_ASSERT|SETTINGS_PERSISTENCE_ASSERT|ObjectDB instances were leaked|Leaked instance:" -Quiet) {
...
try {
    ...
    Invoke-GodotCheck @("--headless", "--path", (Get-Location).Path, "--scene", "res://tests/player-input-integration-test.tscn", "--quit-after", "60") "player-input" "PLAYER_INPUT_INTEGRATION_TEST_OK"
