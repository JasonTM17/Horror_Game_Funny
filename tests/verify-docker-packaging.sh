...
require_no_grep ".github/workflows/docker-suite.yml" '^[[:blank:]]{2}packages:[[:space:]]*write$' "PR suite job never receives package publishing permission"
require_grep ".github/workflows/docker-suite.yml" "ghcr-publish:" "workflow isolates GHCR publishing in a trusted job"
require_grep ".github/workflows/docker-suite.yml" "needs: packaging-and-suite" "GHCR publication waits for the suite"
require_grep ".github/workflows/docker-suite.yml" "tags: \['v\*'\]" "workflow builds release-tag package images"
require_grep ".github/workflows/docker-suite.yml" 'docker run --rm "\$\{GHCR_IMAGE\}:\$\{GITHUB_SHA\}"' "trusted GHCR image runs the suite before publishing"
require_grep ".github/workflows/docker-suite.yml" "ghcr\\.io" "workflow publishes the CI suite to GHCR"
require_grep ".github/workflows/docker-suite.yml" "GITHUB_TOKEN:[[:space:]]+.*github\\.token" "GHCR publish token stays step-scoped"
require_grep ".github/workflows/docker-suite.yml" "GITHUB_REF_NAME" "GHCR release image uses the Git tag"
