...
Require-NoGrep ".github/workflows/docker-suite.yml" '^[ \t]{2}packages:\s*write$' "PR suite job never receives package publishing permission"
Require-Grep ".github/workflows/docker-suite.yml" "ghcr-publish:" "workflow isolates GHCR publishing in a trusted job"
Require-Grep ".github/workflows/docker-suite.yml" "needs: packaging-and-suite" "GHCR publication waits for the suite"
Require-Grep ".github/workflows/docker-suite.yml" "tags: \['v\*'\]" "workflow builds release-tag package images"
Require-Grep ".github/workflows/docker-suite.yml" 'docker run --rm "\$\{GHCR_IMAGE\}:\$\{GITHUB_SHA\}"' "trusted GHCR image runs the suite before publishing"
Require-Grep ".github/workflows/docker-suite.yml" "ghcr\.io" "workflow publishes the CI suite to GHCR"
Require-Grep ".github/workflows/docker-suite.yml" "GITHUB_TOKEN:\s+.*github\.token" "GHCR publish token stays step-scoped"
Require-Grep ".github/workflows/docker-suite.yml" "GITHUB_REF_NAME" "GHCR release image uses the Git tag"
